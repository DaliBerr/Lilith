...
- 地图与寻路：位于 [`Assets/Scripts/Kernel/MapGrid`](Assets/Scripts/Kernel/MapGrid) 与 [`Assets/Scripts/Kernel/MapGridAuthoring.cs`](Assets/Scripts/Kernel/MapGridAuthoring.cs)
  ...
  - [`Assets/Scripts/Kernel/MapGrid/Rooms`](Assets/Scripts/Kernel/MapGrid/Rooms) 是独立的调试版房间地图系统：`RoomGraphGenerator` 只维护本局房间连接和门方向，`ProceduralRoomMapDebugController` 从 [`Assets/Data/Rooms/GenerationProfiles/RoomGenerationProfileLibrary_Default.asset`](Assets/Data/Rooms/GenerationProfiles/RoomGenerationProfileLibrary_Default.asset) 为当前节点选择 `RoomGenerationProfileData`，再调用 `RoomLayoutGenerator` 生成路径优先的约束单房间布局，并交给 `TilemapRoomPresenter` 呈现；profile 资产只控制尺寸、通路、安全区、障碍、敌人出生和房间锚点等生成参数，不定义逐格 tile 模板。生成流程覆盖 `Start / Combat / Reward / Boss`，先保留入口到门和关键锚点的路径，再放置受控障碍、敌人出生点、奖励 / Boss 锚点，并通过 `RoomLayoutValidator` / `RoomLayoutRepair` 收敛到算法生成布局，不再使用 `RoomTemplateData` 作为运行时房间来源。`GeneratedRoomMapRoot` 上的 `ProceduralRoom2DDebugBootstrap` 运行时统一调用 `ProceduralRoomMapDebugController` 生成当前 Tilemap 房间，并把 [`Assets/Prefabs/Player/Test2DCharacterSprite.prefab`](Assets/Prefabs/Player/Test2DCharacterSprite.prefab) 生到第一个玩家入口格中心；2D 调试玩家使用 [`Assets/Scripts/Kernel/Player/Player2DMovementController.cs`](Assets/Scripts/Kernel/Player/Player2DMovementController.cs) 做 XY 平面移动/冲刺，并暴露独立于 `rotateTowardsMouse` 的屏幕空间鼠标朝向；[`Assets/Scripts/Kernel/Player/Player2DMovementAnimatorDriver.cs`](Assets/Scripts/Kernel/Player/Player2DMovementAnimatorDriver.cs) 用移动速度驱动 idle/walk，用屏幕空间鼠标方向驱动视觉正背和左右假镜像；玩家 prefab 当前接入正式 [`Assets/Art/Player/PlayerAnimator/PlayerAnimator.controller`](Assets/Art/Player/PlayerAnimator/PlayerAnimator.controller) 与 `Player.png` 的 `Idle/Walk/Dodge` 正背 clip，移除了 shared `SpriteLibrary` / `SpriteResolver`，不再整张贴图连续旋转；`Main Camera` 上的 [`Assets/Scripts/Kernel/Player/Player2DIsometricCamera.cs`](Assets/Scripts/Kernel/Player/Player2DIsometricCamera.cs) 在运行时接管为沿 Z 轴的透视跟随镜头；当前仍不接入传送门、波次、敌人、攻击、受击、拾取或正式 run flow，旧 3D 地图与 `MapRunFlowController` 链路仍保留
