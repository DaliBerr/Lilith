...
- 攻击与子弹：位于 [`Assets/Scripts/Kernel/Bullet`](Assets/Scripts/Kernel/Bullet)
  ...
  - `ModifierTokenData` 是新 modifier 词元入口，复用 `TokenModifierDefinition` 配置数值载荷，但普通 modifier 不再在资产上写死作用域；编译器会按摆放位置把它解析成 `NextToken`、`NextN` 或 `CurrentBlock`，并遵守“只向后解析、就近最小作用域、payload 内不泄漏到外层”的规则。`Modifier + Value + ...` 会优先把紧随其后的 Value 解释成 `NextN` 目标数；`Modifier + Multicast + ...` 会把 modifier 绑定到这个 upcoming CastBlock；普通 modifier 不会自动推断为 `GlobalProgram`。result-only payload 继续支持用 `ImpactRadiusMultiplier` 修饰 Explosion 半径、Healing 范围与 Control 范围，并用 `ResultCount` / `ResultDuration` / `ResultMultiplier` 修饰 Split 数量、Control 阈值/持续、Explosion 延迟/伤害倍率与 Healing 治疗倍率；`PayloadAmplifyModifier`、`PayloadRadiusModifier`、`PayloadCountModifier` 与 `PayloadControlFieldModifier` 现在只是内容命名上的 payload 样本，不再依赖资产硬编码 `CurrentPayload`
  - `MulticastTokenData` 是当前 CastBlock 的多节点入口：第一个 Multicast 会从右侧收集固定数量 projectile segment，编译成同一个 outer `SpellCastBlock` 下的多个 `SpellProjectileNode`；当前不做 wrapping、result-only 节点或嵌套 Multicast，右侧不足会保留 warning
  ...
  - `TriggerTokenData` 与 `PayloadBoundaryTokenData` 是当前 Trigger/Payload 入口：payload 边界采用显式 `PayloadStart/PayloadEnd` token；`OnHit` payload 可包含普通 inner projectile 或 result-only `SpellPayloadEffectNode`，当前 runtime 覆盖命中后 Explosion、StatusEffect/Control、Healing 与 Split
  - `ValueTokenData` 当前通过消费者 token 声明的 `SpellValueParameterKind` 解释为 Count、Radius 或 Duration；Behavior 的 Spread/Bounce/Pierce 与 Result 的 Split/Control 保留旧 Count 兼容，Explosion 与 Healing 资产已开启 Radius 消费，因此 `爆 + 三` 会把爆炸半径设为 3，`愈 + 三` 会把治疗范围设为 3
  - `CharBullet` 现在会保存当前 `SpellProjectileNode` 的 payload 列表和运行时语义快照；命中 actor 后执行 OnHit payload，非 Bounce 环境命中也会执行 payload，并用最大 payload 深度 / 派生 projectile 数限制防止递归失控；Split 与 result-only Split payload 派生出的子弹会降级为无 payload 的 DirectDamage 子弹，并继续带自己的 `SpellProjectileNode`；Healing result-only payload 在范围为 0 时保持单体治疗，带 Radius 值词或半径 modifier 时会在命中点做范围治疗
  ...
  - Token Select 与背包格已识别新体系 token 类型：Modifier / Multicast / Trigger / Payload 会显示独立目录与颜色；后段波后奖励的 `Plan2` 已接入 [`SpellProgram_Token_Lib.asset`](Assets/Data/BulletTokens/TokenLib/SpellProgram_Token_Lib.asset)，也能通过 [`SpellBookReward_Lib.asset`](Assets/Data/SpellBooks/SpellBookReward_Lib.asset) 抽到法术书奖励
...
- Bullet Token 数据：[`Assets/Data/BulletTokens`](Assets/Data/BulletTokens)
  ...
  - 新体系 token 第一批资产位于 `Modifier`、`Multicast`、`Trigger`、`Payload` 子目录；[`SpellProgram_Token_Lib.asset`](Assets/Data/BulletTokens/TokenLib/SpellProgram_Token_Lib.asset) 会混合 Core、Value、Modifier、Multicast、Trigger 与 Payload Boundary，其中 Modifier 样本包含 `HasteModifier`、`BlockAmplifyModifier`，以及面向 payload result-only 效果的 `PayloadAmplifyModifier` / `PayloadRadiusModifier` / `PayloadCountModifier` / `PayloadControlFieldModifier`
...
- 法术书数据：[`Assets/Data/SpellBooks`](Assets/Data/SpellBooks)
  ...
  - [`WideSpellBook.asset`](Assets/Data/SpellBooks/WideSpellBook.asset)、[`QuickSpellBook.asset`](Assets/Data/SpellBooks/QuickSpellBook.asset)、[`TriggerSpellBook.asset`](Assets/Data/SpellBooks/TriggerSpellBook.asset)、[`SurgeSpellBook.asset`](Assets/Data/SpellBooks/SurgeSpellBook.asset)、[`BindingSpellBook.asset`](Assets/Data/SpellBooks/BindingSpellBook.asset) 是第一批可奖励法术书：Wide 固定前置 `BlockAmplifyModifier`、提供 7 槽慢冷却，并以 10 度激活扇形每次发射 2 轮；Quick 固定前置 `HasteModifier`、提供 4 槽快冷却，并带原生伤害 `*=0.85`；Trigger 固定后置 `OnHitTrigger + PayloadStart + Explosion + PayloadEnd`，会把玩家装备的外层法术变成命中触发爆炸 payload 的构筑，并带原生 payload 结果倍率 `*=1.25`；Surge 提供 5 槽、0.18 秒冷却、每次 3 轮 8 度扇形，带 3 点能量容量、每次消耗 1、每秒恢复 0.75 的爆发资源门槛，并带原生伤害 `*=0.8`；Binding 提供 5 槽、0.32 秒中速冷却，固定后置 `OnHitTrigger + PayloadStart + Control + PayloadEnd`，会把玩家装备的外层法术变成命中触发控制 payload 的构筑，并用原生 `ResultCount =1` / `ResultDuration *=1.5` 把控制阈值降为 1 次、持续时间放大到 1.5 倍
