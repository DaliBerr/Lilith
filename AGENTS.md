...
2) 尽量减少无关文件对注意力的消耗。
3) 在不确定时，优先向我索要 **具体脚本路径/报错信息/相关片段**，不要盲目扫描整个仓库。
---
...
9) **分层边界是硬规则**
   - 禁止在 `Assets/Scripts/Lonize/**` 新增任何 `Kernel.*` 引用
   - 若基础设施需要游戏语义，必须改为在 `Kernel` 增加 adapter / extension / bridge，或将抽象下沉到 `Lonize`
   - 当前其他 `Lonize` 子模块若仍存在历史反向依赖，属于技术债，不代表规则例外
