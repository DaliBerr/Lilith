...
这些测试确认当前 Trigger/Payload 采用隐式边界：`TriggerTokenData(OnHit)` 后续 token 自动构成 payload，并把 payload block 绑定到外层 projectile；非 Multicast 公式中 payload 消费到整式末尾，Multicast 中只属于当前 projectile segment。payload 内无 Core 时可编译 result-only `SpellPayloadEffectNode`，当前覆盖 Explosion、StatusEffect/Control、Healing 与 Split。result-only payload modifier 首批映射支持 `ImpactRadiusMultiplier` 修饰 Explosion 半径、Healing 范围与 Control 范围，并支持 `ResultCount` / `ResultDuration` / `ResultMultiplier` 改写 Split 数量、Control 阈值/持续、Explosion 延迟/伤害倍率与 Healing 治疗倍率；法术书 executor modifier 也会用同一套 result 目标映射作用到 result-only payload effect，`TriggerSpellBook` 的原生 `ResultMultiplier *=1.25` 是 Explosion payload 的资产级护栏，`BindingSpellBook` 的原生 `ResultCount =1` / `ResultDuration *=1.5` 是 Control payload 的资产级护栏。Healing 资产已声明 Radius 值词消费，`愈 + 三` 可把治疗范围写为 3；运行时范围 Healing payload 会治疗命中点附近目标，半径为 0 时保持旧的主目标单体治疗；Control payload 半径为 0 时保持单体控制，半径大于 0 时会在命中点附近对合法敌人登记控制命中。当前可抽取 token 资产级护栏有四个：`PayloadAmplifyModifier` 以 payload 局部 `ResultMultiplier *=1.5` 放大当前 payload 内 result-only Explosion 伤害倍率并保持半径不变，`PayloadRadiusModifier` 以 payload 局部 `ImpactRadiusMultiplier *=1.35` 放大 Explosion / Healing 半径并保持伤害/治疗倍率不变，`PayloadCountModifier` 以 payload 局部 `ResultCount +=2` 增加 result-only Split 数量并保持 Split 子弹伤害倍率不变，`PayloadControlFieldModifier` 以 payload 局部 `ImpactRadiusMultiplier =1.25` 把 result-only Control 扩展为范围控制。发射器会把 `SpellProjectileNode` 注入到 `CharBullet`，命中后 `CharBullet` 会执行 OnHit payload。当前不启用嵌套 payload / trigger，也不做 payload wrapping。
