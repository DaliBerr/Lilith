...
这些 UI 已经使用固定屏幕像素边框组件。你可以把它理解成普通 Unity `Image`，但它会在玩家调大 UI 缩放时尽量保持边框像素厚度不变。真实边框图接入时，最重要的是正确设置 Sprite 的九宫格 Border。
...
| --- | --- |
| 图片格式 | PNG，透明背景 |
| 像素风 UI | 保持硬边，不要加半透明模糊抗锯齿 |
| 外框厚度 | 先按最终画面希望看到的像素厚度绘制，例如 4px、5px 或 6px |
| 四角区域 | 四角不要依赖拉伸；角上的装饰必须完整放进 Border 区域 |
| 边线区域 | 横边只允许横向重复 / 拉伸后仍可接受；竖边同理 |
| 中心区域 | 可留透明、纯色或可拉伸底纹；不要放不能被拉伸的花纹 |
| 命名 | 建议 `ui_frame_panel.png`、`ui_frame_button.png`、`ui_frame_dropdown.png` 这类可读名称 |

如果所有 UI 暂时继续共用一种边框，可以直接替换 `Assets/Images/dialog-box.png` 的图片内容，并尽量保留原 `.meta` 文件。这样现有 UI prefab 会自动使用新图。

如果按钮、面板、下拉框、滑块等需要不同边框，请新增独立图片，建议放在：

`Assets/Images/UI/Frames/`

新增多张图后，不要自己大范围替换 prefab；先挑 1 到 2 个代表性 UI 交给程序同事接入和验证，确认规则稳定后再批量换。

#### Unity 导入设置

选中图片后，在 Inspector 里建议这样设置：
...
如果不是像素风图片，可以和程序同事确认是否改用 `Bilinear`。当前 UI 更偏像素风时，优先使用 `Point`。

#### Sprite Editor 里的 Border 怎么设

1. 选中边框 PNG。
2. 打开 `Sprite Editor`。
3. 设置 `L`、`R`、`T`、`B` 四个 Border 数值。
4. 点击 `Apply`。
...
当前项目的固定像素边框组件会读取这里的 Border。也就是说，如果你把边框设为 5px，它在不同 UI 缩放倍率下都会尽量保持约 5 个屏幕像素厚。

Border 应该包含完整边框装饰，而不只是最外面黑线。例如一条边由黑线、亮边、高光三层组成，总共 7px，那么 Border 通常也应该至少设到 7px。

#### 快速自检

替换或新增 UI 边框图后，请至少看这几个地方：

| 检查项 | 通过标准 |
| --- | --- |
| 小按钮 | 四角没有被拉歪，文字没有压到边框 |
| 大面板 | 横边 / 竖边拉伸后没有明显糊、断、错位 |
| 下拉框 | 右侧箭头区域不被边框吃掉 |
| 滑块 handle | 小尺寸时边框不交叉、不变成黑块 |
| 1.0x 和 1.5x UI 缩放 | 控件变大，但边框视觉厚度接近一致 |
| 16:9 分辨率 | 主要 UI 不被裁切，不遮挡关键文字 |

如果发现某个控件边框过厚或边角挤在一起，优先检查 Sprite Editor 的 Border 数值，而不是改控件缩放。

#### 替换时不要动

| 不要动 | 原因 |
| --- | --- |
| UI prefab 上的脚本组件 | 会影响界面打开、关闭和数据绑定 |
| `Fixed Pixel Sliced Image` / `Image` 的组件引用关系 | Button、Toggle、Selectable 可能引用它作为点击目标 |
| `Type: Sliced` | 九宫格边框需要这个模式 |
| 已经拖好的 `Target Graphic` | 清空后按钮可能还能显示，但交互状态会异常 |
| Layout、Grid、Content Size Fitter | 随意改可能导致不同分辨率下排版错乱 |

美术同学可以安全调整的内容通常是：Sprite、Color、材质、图片导入设置、Sprite Border，以及纯视觉子物体。
